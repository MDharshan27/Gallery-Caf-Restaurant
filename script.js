document.addEventListener('DOMContentLoaded', () => {
    ...
    document.querySelector('.prev-btn').addEventListener('click', () => {
        if (currentIndex === 0) {
            currentIndex = cardCount / 2 - 1; // Jump to the middle to create infinite effect
            ...
            setTimeout(() => {
                ...
                currentIndex--;
                updateSlider();
            }, 50); // Short delay to apply the transition
        } else {
            currentIndex--;
            updateSlider();
        }
    ...
    document.querySelector('.next-btn').addEventListener('click', () => {
        if (currentIndex === cardCount / 2 - 1) {
            currentIndex = 0; // Jump back to the start
            slider.style.transition = 'none'; // Disable transition
            updateSlider();
            setTimeout(() => {
                slider.style.transition = 'transform 0.5s ease-in-out'; // Re-enable transition
...
function highlightFood() {
    ...
    if (food) {
        ...
        if (foodItem) {
            ...
            // Find the button inside the highlighted food item and change its color
            const button = foodItem.querySelector('button');
            if (button) {
                button.classList.add('button-highlight'); // Apply a custom class for button highlighting
            }
        }
    }
}
