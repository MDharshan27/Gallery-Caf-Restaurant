...
<head>
    ...
    <title>Gallery Cafe Restaurant - Beverages</title>
...
<section class="food-menu">
    <div class="container-f">
        <h2>Find our</h2>
        <h1>Most Popular Beverage Items</h1>
    </div>
    ...
    <?php
        ...
        $menu_items = $conn->query("SELECT * FROM beverage_menu ORDER BY id ASC");
        ?>

        <div class="menu-content breakfast">
            <?php while ($item = $menu_items->fetch_assoc()) { ?>
                <div class="menu-card">
                    <img src="Images/Beverages/<?php echo $item['image']; ?>" alt="<?php echo $item['name']; ?>">
                    ...
                    <div class="menu-card-price">
                        ...
                        <button onclick="addToCart('<?php echo $item['name']; ?>', <?php echo $item['price']; ?>, 'Images/Beverages/<?php echo $item['image']; ?>')">Add to Cart</button>
                    </div>
                </div>
            <?php } ?>
        </div>
...
<?php
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['email'])) {
        ...
        $conn = new mysqli($servername, $username, $password, $newsletter_db);

        // Check connection
        if ($conn->connect_error) {
            $isSuccess = false;
            $message = "Connection to newsletter_db failed: " . $conn->connect_error;
        } else {
